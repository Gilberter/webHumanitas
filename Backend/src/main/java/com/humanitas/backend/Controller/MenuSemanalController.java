...
public class MenuSemanalController {
    ...
    @PostMapping
    public ResponseEntity<?> crearItemMenu(@RequestBody MenuSemanal menuItem) {
        try {
    ...
    // TODO: Asegurar con Spring Security - @PreAuthorize("hasRole('ADMINISTRADOR')")
    @PutMapping("/{id}")
    public ResponseEntity<?> actualizarItemMenu(@PathVariable int id, @RequestBody MenuSemanal menuActualizado) {
        try {
